...
const Contact = () => {
  ...
  const onSubmit = async (event) => {
    ...
    formData.append("access_key", "062a1cf2-a7f1-485f-88f1-56d332cfaec5");
  ...
  return (
    ...
    initial={{opacity: 0}} whileInView={{opacity: 1}} transition={{duration: 1}}>
      ...
      initial={{opacity: 0}} whileInView={{opacity: 1}} transition={{duration: 0.5, delay: 0.9}}>
        ...
        type='submit' className='py-3 px-8 w-max flex items-center justify-between gap-2 bg-black/80 text-white rounded-full mx-auto hover:bg-black duration-500 dark:bg-darkTheme dark:border dark:hover:bg-darkHover '>
          Submit
          <FaArrowRight className='w-4 '/> 
        </motion.button>

        <p className='mt-4 font-semibold italic'>{result}</p>
      </motion.form>
